...
def martelo_alta(df, p):
    ...
    if name not in df.columns:
        df[name] = (df['Open'].shift(abs(bar)) < df['Close'].shift(abs(bar))) & \
                   ...
                   ((df['Close'].shift(abs(bar)) - df['Open'].shift(abs(bar))) <= (
                               df['Open'].shift(abs(bar)) - df['Low'].shift(abs(bar))) / 2)
...
def martelo_baixa(df, p):
    ...
    if name not in df.columns:
        df[name] = (df['Open'].shift(abs(bar)) > df['Close'].shift(abs(bar))) & \
                   ...
                   ((df['Open'].shift(abs(bar)) - df['Close'].shift(abs(bar))) <= (
                               df['Close'].shift(abs(bar)) - df['Low'].shift(abs(bar))) / 2)
...
def estrela_cadente_alta(df, p):
    ...
    if name not in df.columns:
        df[name] = (df['Open'].shift(abs(bar)) < df['Close'].shift(abs(bar))) & \
                   (df['Open'].shift(abs(bar)) == df['High'].shift(abs(bar))) & \
                   ((df['Close'].shift(abs(bar)) - df['Open'].shift(abs(bar))) <= (
                               df['High'].shift(abs(bar)) - df['Close'].shift(abs(bar))) / 2)
...
def estrela_cadente_baixa(df, p):
    ...
    if name not in df.columns:
        df[name] = (df['Open'].shift(abs(bar)) > df['Close'].shift(abs(bar))) & \
                   (df['Close'].shift(abs(bar)) == df['High'].shift(abs(bar))) & \
                   ((df['Open'].shift(abs(bar)) - df['Close'].shift(abs(bar))) <= (
                               df['High'].shift(abs(bar)) - df['Open'].shift(abs(bar))) / 2)
...
def piercing_line(df, p):
    ...
    if name not in df.columns:
        df[name] = (df['Close'].shift(abs(bar)) > df['Open'].shift(abs(bar))) & \
                   (df['Open'].shift(abs(bar)) < df['Close'].shift(1 + abs(bar))) & \
                   (df['Close'].shift(abs(bar)) >= (
                               df['Close'].shift(1 + abs(bar)) + df['Open'].shift(1 + abs(bar))) / 2) & \
...
def harami_de_fundo(df, p):
    ...
    if name not in df.columns:
        df[name] = (df['Close'].shift(abs(bar)) > df['Open'].shift(abs(bar))) & \
                   ...
                   ((df['Close'].shift(abs(bar)) - df['Open'].shift(abs(bar))) <= (
                               (df['Open'].shift(1 + abs(bar)) - df['Close'].shift(1 + abs(bar))) / 4))
...
def harami_de_topo(df, p):
    ...
    if name not in df.columns:
        df[name] = (df['Close'].shift(abs(bar)) < df['Open'].shift(abs(bar))) & \
                   ...
                   (df['Close'].shift(abs(bar)) > df['Open'].shift(1 + abs(bar))) & \
                   ((df['Open'].shift(abs(bar)) - df['Close'].shift(abs(bar))) <= (
                               (df['Close'].shift(1 + abs(bar)) - df['Open'].shift(1 + abs(bar))) / 4))
...
def estrela_da_manha(df, p):
    ...
    if name not in df.columns:
        df[name] = (df['Close'].shift(abs(bar)) > df['Open'].shift(abs(bar))) & \
                   ...
                   ((df['Open'].shift(1 + abs(bar)) - df['Close'].shift(1 + abs(bar))) <= (
                               (df['Open'].shift(2 + abs(bar)) - df['Close'].shift(2 + abs(bar))) / 4))
...
def fech_cruza_cima_med(df, p):
    ...
    if name not in df.columns:
        df[name] = ((df['Close'].shift(abs(bar)) - df[med].shift(abs(bar))) > 0.) & (
                    (df['Close'].shift(1 + abs(bar)) - df[med].shift(1 + abs(bar))) < 0.)
...
def med1_cruza_cima_med2(df, p):
    ...
    if name not in df.columns:
        df[name] = ((df[med1].shift(abs(bar)) - df[med2].shift(abs(bar))) > 0.) & (
                    (df[med1].shift(1 + abs(bar)) - df[med2].shift(1 + abs(bar))) < 0.)
...
def percentB_maior(df, p):
    ...
    if name not in df.columns:
        df[name] = (np.where(df[bs].shift(abs(bar)) == df[bi].shift(abs(bar)), 50.0, \
                             100 * (df['Close'].shift(abs(bar)) - df[bi].shift(abs(bar))) / (
                                         df[bs].shift(abs(bar)) - df[bi].shift(abs(bar))))) >= pX
...
def macd_cruza_sinal_cima(df, p):
    ...
    if name not in df.columns:
        df[name] = (df[macd].shift(abs(bar)) > df[sinal].shift(abs(bar))) & (
                    df[macd].shift(1 + abs(bar)) < df[sinal].shift(1 + abs(bar)))
...
def hist_macd_topo(df, p):
    ...
    if name not in df.columns:
        df[name] = (df[hist].shift(1 + abs(bar)) > df[hist].shift(abs(bar))) & (
                    df[hist].shift(1 + abs(bar)) > df[hist].shift(2 + abs(bar)))
...
def tr_dm(df):
    ...
    for i in range(0, df.index[-1]):
        if i < 1:
            ...
            tr = max((df.loc[i, 'High'] - df.loc[i, 'Low']), abs(df.loc[i, 'High'] - df.loc[i - 1, 'Close']),
...
def DIP_cruza_cima_DIN(df, p):
    ...
    if name not in df.columns:
        df[name] = (df[dip].shift(abs(bar)) > df[din].shift(abs(bar))) & (
                    df[dip].shift(1 + abs(bar)) < df[din].shift(1 + abs(bar)))
...
def K_cruza_cima_D(df, p):
    ...
    if kf not in df.columns:
        df[kf] = 100 * (df['Close'] - df['Low'].rolling(center=False, window=pKF).min()) / (
                    df['High'].rolling(center=False, window=pKF).max() - df['Low'].rolling(center=False,
                                                                                           window=pKF).min())
    ...
    if name not in df.columns:
        df[name] = (df[ks].shift(abs(bar)) > df[ds].shift(abs(bar))) & (
                    df[ks].shift(1 + abs(bar)) < df[ds].shift(1 + abs(bar)))
...
def vol_cruza_cima_med(df, p):
    ...
    if name not in df.columns:
        df[name] = (df['Volume'].shift(abs(bar)) > df[med].shift(abs(bar))) & (
                    df['Volume'].shift(1 + abs(bar)) < df[med].shift(1 + abs(bar)))
...
def didi_agulhada_compra(df, p):
    ...
    if name not in df.columns:
        df[name] = (df[dif12].shift(abs(bar)) > 0) & (df[dif32].shift(abs(bar)) < 0) & (
                    df[dif12].shift(1 + abs(bar)) < 0) & (df[dif32].shift(1 + abs(bar)) > 0)
